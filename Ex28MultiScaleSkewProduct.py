...
def fgen(epsilon,alpha,lambda_):
    def f_exact(x):
        return np.array(((1-x[:,1]**2)*x[:,0],-alpha/epsilon*x[:,1])).T
    def diff_exact(x):
        sigma = np.array(((0,0),(0,np.sqrt(2*lambda_/epsilon))))
        return np.repeat(sigma[None,:,:],x.shape[0],axis=0)
    # def f_exact(x):
    #     return np.array(((1-x[:,1]**2)*x[:,0]/epsilon,-alpha/epsilon**2*x[:,1])).T
    # def diff_exact(x):
    #     sigma = np.array(((0,0),(0,np.sqrt(2*alpha/epsilon**2))))
    #     return np.repeat(sigma[None,:,:],x.shape[0],axis=0)
...
def Gendata(T,Nt,N_data,ifrandom,epsilon,multi,reduced=False):
    ...
    if ifrandom:
        ...
        xIC = np.array((1.5*np.ones(N_data),np.random.normal(0,np.sqrt(lambda_/alpha),N_data))).T
...
if __name__ == '__main__':
    ...
    data_train = Gendata(T=1.0, Nt=100, N_data=10000,  ifrandom=True,  epsilon=0.005, multi=100, reduced=True)
    data_test  = Gendata(T=4.0, Nt=400, N_data=10000,  ifrandom=False, epsilon=0.005, multi=100, reduced=True)
