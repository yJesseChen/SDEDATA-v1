...
def fgen(epsilon,sigma_):
    def f_exact(x):
        return np.array((-2/epsilon*x[:,1]*x[:,2],
                         -1/epsilon**2*x[:,1]+1/epsilon*x[:,0]*x[:,2],
                         -2/epsilon**2*x[:,2]+1/epsilon*x[:,0]*x[:,1])).T
    def diff_exact(x):
...
def Gendata(T,Nt,N_data,ifrandom,reduced=False):
    ...
    epsilon = np.sqrt(0.1)
    sigma_ = 1.0
    f_exact,diff_exact = fgen(epsilon,sigma_)
    dim = 3
    # initial condition - can be changed
    if ifrandom:
        # xIC = np.random.uniform(-2,2,N_data)
        xIC = np.array((np.random.uniform(-2.0,3.0,N_data),np.random.uniform(-1.0,1.0,N_data),np.random.uniform(-1.0,1.0,N_data))).T
    else:
        # xIC = np.array((1.5*np.ones(N_data),2.0*np.ones(N_data))).T
        xIC = np.array((1.0*np.ones(N_data),np.random.normal(0,np.sqrt(0.5),N_data),np.random.normal(0,0.5,N_data))).T
...
if __name__ == '__main__':
    ...
    data_train = Gendata(T=10.0, Nt=1000, N_data=10000,  ifrandom=True,  reduced=True)
    data_test  = Gendata(T=10.0, Nt=1000, N_data=10000,  ifrandom=False, reduced=True)
