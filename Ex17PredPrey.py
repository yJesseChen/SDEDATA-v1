...
def Gendata(T,Nt,N_data,ifrandom,ifpara):
    ...
    if ifrandom:
        ...
        xIC = np.array((np.random.uniform(0.1,3.5,N_data),np.random.uniform(0.2,5.5,N_data))).T
        # xIC = np.array((np.random.uniform(0.1,3.7,N_data),np.random.uniform(0.2,6.5,N_data))).T
    else:
    ...
    if not ifpara:
        ...
        dim_para     = 3
        if ifrandom:
            para_data = np.array((np.random.uniform(0.01,4.2,[Nt,N_data]),np.random.uniform(-1.5,1.5,[Nt,N_data]),np.random.uniform(-0.7,0.7,[Nt,N_data])))
...
if __name__ == '__main__':
    ...
    data_test , para_test  = Gendata(T=80.0,   Nt=8000,  N_data=10000,  ifrandom=False,ifpara=True)
