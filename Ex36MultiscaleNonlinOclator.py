...
def fgen(epsilon,lambda_,f_,gamma,sigma_):
    def f_exact(x):
        return np.array((lambda_*x[:,0]-f_*x[:,1]-gamma*x[:,0]*x[:,2],
                         f_*x[:,0]+lambda_*x[:,1]-gamma*x[:,1]*x[:,2],
                        -1/epsilon*(x[:,2]-x[:,0]**2-x[:,1]**2))).T
    def diff_exact(x):
        sigma = np.array(((0,0,0),
                        (0,0,0),
                        (0,0,sigma_*np.sqrt(2)/np.sqrt(epsilon))))
...
def Gendata(T,Nt,N_data,ifrandom,reduced=False):
    ...
    gamma = 1.0
    sigma_ = 1.0
    f_exact,diff_exact = fgen(epsilon,lambda_,f_,gamma,sigma_)
    dim = 3
    # initial condition - can be changed
    if ifrandom:
        # xIC = np.random.uniform(-2,2,N_data)
        xIC = np.array((np.random.uniform(-1.5,1.5,N_data),np.random.uniform(-1.5,1.5,N_data),np.random.uniform(0.1,2.5,N_data))).T
    else:
        # xIC = np.array((1.5*np.ones(N_data),2.0*np.ones(N_data))).T
        xIC = np.array((1.0*np.ones(N_data),1.0*np.ones(N_data),np.random.normal(2,sigma_,N_data))).T
        # xIC = np.array((0.001*np.ones(N_data),0.0*np.ones(N_data),np.random.normal(0.0,sigma_,N_data))).T
...
if __name__ == '__main__':
    ...
    data_train = Gendata(T=50.0, Nt=500, N_data=500,  ifrandom=True,  reduced=False)
    data_test  = Gendata(T=50.0, Nt=500, N_data=500,  ifrandom=False, reduced=False)
