...
def fgen(mu,omega,sigma,V):
    def b(t):
        return V*np.sin(omega*t)
    # def b(t):
    #     return V*np.sin(omega*t)+0.4*np.sin(3*omega*t)
    # def b(t):
    #     # b2
    #     return V*np.sin(omega/1.2*t)+0.2*np.sin(omega/4*t)
    # def b(t):
...
def Gendata(T,Nt,N_data,ifrandom,ifpara):
    ...
    omega = 6.0
    V     = 0.5
    f_exact,f_appx,b,diff_exact,diff_appx = fgen(mu,omega,sigma,V)
    # initial condition - can be changed
    if ifrandom:
        # xIC = np.random.uniform(-2,2,N_data)
        xIC = np.random.uniform(-5,5,N_data)
    else:
        xIC = 0.0*np.ones(N_data)
    ...
    if not ifpara:
        ...
        dim_para     = 3
        # if ifrandom:
        #     para_data = np.random.uniform(-9,9,[3,Nt,N_data])
...
if __name__ == '__main__':
    ...
    data_train, para_train = Gendata(T=0.01,    Nt=1,      N_data=1000000, ifrandom=True, ifpara=True)
    data_test , para_test  = Gendata(T=10.0,    Nt=1000,   N_data=5000,    ifrandom=False,ifpara=True)
