...
def EMgamma(x0,para,f,diff,tseq):
    ...
    re[:,0] = x0
    noise = std_normal(x0.shape[0], tseq)
    a_ = gamma2(dt,para[0].T,para[1].T,para[2].T)
    b_ = gamma2(dt,para[3].T,para[4].T,para[5].T)
    for i in range(numT-1):
        re[:,i+1] = re[:,i]+dtseq[i]*f(re[:,i],a_[:,i])+diff(re[:,i],b_[:,i])*noise[:,i+1]
    return re

def fgen(mu,sigma,V,omega,omega2):
    # def a(t):
    #     return V*np.sin(omega2*t)
    # def a(t):
    #     # a2
    #     return V/2*np.sin(omega2*t)**2
    def a(t):
        # a3
        return V/2*np.sin(2*omega2*t)
    # def b(t):
    #     return sigma*(np.sin(omega*t)+2)
    # def b(t):
    #     # b2
    #     return sigma*np.exp(np.cos(omega*t))
    # def b(t):
    #     # b3
    #     return sigma*(np.arctan(t/omega)+0.5)
    def b(t):
        # b4
        return sigma*(np.cos(omega*t)+2)
    def f_exact(x,t):
        return -mu*x+a(t)
    def f_appx(x,a_):
        return -mu*x+a_
    ...
    def diff_appx(x,b_):
        return b_
    return f_exact,f_appx,a,b,diff_exact,diff_appx
...
def Gamma2(tsq,a,b):
    ...
    numT = tsq.shape[0]
    dt = tsq[1]-tsq[0]
    re = np.zeros([6,numT-1])
    V  = np.vander(np.array((0,dt/2,dt)), increasing=True)
    Vi = np.linalg.inv(V)
    for i in range(numT-1):
        re[:3,i] = np.dot(Vi,a(np.array((tsq[i],tsq[i]+dt/2,tsq[i]+dt))))
        re[3:,i] = np.dot(Vi,b(np.array((tsq[i],tsq[i]+dt/2,tsq[i]+dt))))
    return re
...
def Gendata(T,Nt,N_data,ifrandom,ifpara):
    ...
    omega2 = np.pi/2
    V = 0.5
    f_exact,f_appx,a,b,diff_exact,diff_appx = fgen(mu,sigma,V,omega,omega2)
    # initial condition - can be changed
    if ifrandom:
        # xIC = np.random.uniform(-2,2,N_data)
        # xIC = np.random.uniform(-0.5,1.3,N_data)
        # xIC = np.random.uniform(-2.0,2.3,N_data)
        xIC = np.random.uniform(-0.8,1.5,N_data)
    else:
        xIC = 0.7*np.ones(N_data)
    ...
    if not ifpara:
        ...
        dim_para     = 6
        if ifrandom:
            # para_data = np.array((np.random.uniform(-0.6,0.6,[Nt,N_data]),np.random.uniform(-0.8,0.8,[Nt,N_data]),np.random.uniform(-0.7,0.7,[Nt,N_data]),\
            #                       np.random.uniform(0.05,0.35,[Nt,N_data]),np.random.uniform(-0.35,0.35,[Nt,N_data]),np.random.uniform(-0.55,0.55,[Nt,N_data])))
            # para_data = np.array((np.random.uniform(-0.6,0.6,[Nt,N_data]),np.random.uniform(-0.8,0.8,[Nt,N_data]),np.random.uniform(-0.7,0.7,[Nt,N_data]),\
            #                       np.random.uniform(0.25,0.95,[Nt,N_data]),np.random.uniform(-1.0,1.0,[Nt,N_data]),np.random.uniform(-1.5,1.5,[Nt,N_data])))
            para_data = np.array((np.random.uniform(-0.6,0.6,[Nt,N_data]),np.random.uniform(-0.8,0.8,[Nt,N_data]),np.random.uniform(-0.7,0.7,[Nt,N_data]),\
                                  np.random.uniform(0.01,0.35,[Nt,N_data]),np.random.uniform(-0.5,0.5,[Nt,N_data]),np.random.uniform(-1.55,0.55,[Nt,N_data])))
...
if __name__ == '__main__':
    ...
    data_train, para_train = Gendata(T=0.01,    Nt=1,    N_data=200000, ifrandom=True, ifpara=True)
